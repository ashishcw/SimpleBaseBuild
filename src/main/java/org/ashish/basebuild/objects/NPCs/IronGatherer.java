...
public class IronGatherer extends GameObjectModelMain {
    ...
    IronResource ironResource = null;
    HomeBase homeBase = null;


    private float gatheringRate = 0.5f, dumpingRate = 0.5f, ironResourceCollected = 0.0f;
    private float ironResourcesMAX = 20.0f;
    ...
    public IronGatherer(int xPos, int yPos, ResourceType.RawResourceType rawResourceTypeParam) {
        super(xPos, yPos);
        ...
        this.setName("Iron-Resource-Gatherer");
        ...
        for(var ironResourceLocation : MainHandler.allObjectsList){
            if(ironResourceLocation.getName() != null && ironResourceLocation.getName().contains("Iron-Resource")){
                ironResource = (IronResource) ironResourceLocation;
            }

            if(ironResourceLocation.getName() != null && ironResourceLocation.getName().contains("Home-Base")){
                this.homeBase = (HomeBase) ironResourceLocation;
            }

            if(this.ironResource != null && this.homeBase != null){
                break;
            }
        }
    ...
    public void tick() {
        ...
        if(isOnTheWayToCollectResources){
            if(ironResource != null){
                ...
                goalTarget = aStarPathfinding.search(Node.nodes[this.getRow()][this.getCol()], Node.nodes[ironResource.getRow()-1][ironResource.getCol()]);
                isOnTheWayToCollectResources = false;
            }
        }else {
            if(MathHelper.isColliding(this.getHitBox(), ironResource.getHitBox()) && this.ironResourceCollected < this.ironResourcesMAX){
...
//                System.out.println("Water Collected : " + this.waterResourceCollected);
                ...
                if(this.ironResourceCollected < this.ironResourcesMAX){
                    this.ironResourceCollected += this.gatheringRate;
                    this.ironResourceCollected = MathHelper.ClampFloat(this.ironResourceCollected, this.ironResourcesMAX, 0.0f);
                    //System.out.println("Water Collected : " + this.waterResourceCollected);
        ...
        if(isOnTheWayToDumpResources){
            ...
            if(MathHelper.isColliding(this.getHitBox(), this.homeBase.getHitBox()) && this.ironResourceCollected >= this.ironResourcesMAX){
        ...
        if(isDumpingResources){
            if((this.waitTimeBeforeAction+100) < System.currentTimeMillis()){
                if(this.ironResourceCollected > 0){
                    this.ironResourceCollected -= this.gatheringRate;
                    this.ironResourceCollected = MathHelper.ClampFloat(this.ironResourceCollected, this.ironResourcesMAX, 0.0f);
                }else{
                    ResourceManager.COLLECTED_IRON += this.ironResourcesMAX;
                    System.out.println("Iron Collected : " + ResourceManager.COLLECTED_IRON);
