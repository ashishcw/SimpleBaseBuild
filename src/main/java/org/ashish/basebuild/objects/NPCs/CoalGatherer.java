...
public class CoalGatherer extends GameObjectModelMain {
    ...
    CoalResource coalResource = null;
    HomeBase homeBase = null;


    private float gatheringRate = 0.5f, dumpingRate = 0.5f, coalResourceCollected = 0.0f;
    private float coalResourcesMAX = 20.0f;
    ...
    public CoalGatherer(int xPos, int yPos, ResourceType.RawResourceType rawResourceTypeParam) {
        super(xPos, yPos);
        ...
        this.setName("Coal-Resource-Gatherer");
        ...
        for(var coalResourceLocation : MainHandler.allObjectsList){
            if(coalResourceLocation.getName() != null && coalResourceLocation.getName().contains("Coal-Resource")){
                coalResource = (CoalResource) coalResourceLocation;
            }

            if(coalResourceLocation.getName() != null && coalResourceLocation.getName().contains("Home-Base")){
                this.homeBase = (HomeBase) coalResourceLocation;
            }

            if(this.coalResource != null && this.homeBase != null){
                break;
            }
        }
    ...
    public void tick() {
        ...
        if(isOnTheWayToCollectResources){
            if(coalResource != null){
                //System.out.println(waterResource);
                goalTarget = aStarPathfinding.search(Node.nodes[this.getRow()][this.getCol()], Node.nodes[coalResource.getRow()-1][coalResource.getCol()]);
                isOnTheWayToCollectResources = false;
            }
        }else {
            if(MathHelper.isColliding(this.getHitBox(), coalResource.getHitBox()) && this.coalResourceCollected < this.coalResourcesMAX){
...
//                System.out.println("Water Collected : " + this.waterResourceCollected);
                ...
                if(this.coalResourceCollected < this.coalResourcesMAX){
                    this.coalResourceCollected += this.gatheringRate;
                    this.coalResourceCollected = MathHelper.ClampFloat(this.coalResourceCollected, this.coalResourcesMAX, 0.0f);
                    //System.out.println("Water Collected : " + this.waterResourceCollected);
        ...
        if(isOnTheWayToDumpResources){
            ...
            if(MathHelper.isColliding(this.getHitBox(), this.homeBase.getHitBox()) && this.coalResourceCollected >= this.coalResourcesMAX){
        ...
        if(isDumpingResources){
            if((this.waitTimeBeforeAction+100) < System.currentTimeMillis()){
                if(this.coalResourceCollected > 0){
                    this.coalResourceCollected -= this.gatheringRate;
                    this.coalResourceCollected = MathHelper.ClampFloat(this.coalResourceCollected, this.coalResourcesMAX, 0.0f);
                }else{
                    ResourceManager.COLLECTED_COAL += this.coalResourcesMAX;
                    System.out.println("Coal Collected : " + ResourceManager.COLLECTED_COAL);
